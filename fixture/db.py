...
class DbFixture:
    ...
    def get_userinfo_tel_list(self):
        ...
        try:
            ...
            for row in cursor:
                ...
                list.append(UserInfo(id=str(id), firstname=firstname, lastname=lastname, homephone=home, mobilephone=mobile, workphone=work, phone2=phone2))
        finally:
            cursor.close()
        return list
    ...
    def get_userinfo_group_list(self):
        list=[]
        cursor = self.connection.cursor()
        try:
            cursor.execute("select id, firstname, lastname, email, email2, email3 from addressbook where deprecated='0000-00-00 00:00'")
